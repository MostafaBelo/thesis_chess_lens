...
class CornerExtractor:
    ...
    def detectEdges(self):
        ...
        self.edges = cv2.Canny(
            self.img, self.EdgeThresholds[0], self.EdgeThresholds[1])
    ...
    def fitBoard(self, oriented_centrals=None, grid_size=4):
        ...
        for grid_shape in range(grid_size, 9):
            for i in range(9-grid_shape):
                for j in range(9-grid_shape):
                    simulated = torch.tensor([
                        [i, j],  # TL
                        [(grid_shape + i), j],  # TR
                        [(grid_shape+i), (grid_shape+j)],  # BR
                        [i, (grid_shape+j)],  # BL
        ...
        return best_M, best_mask, best_error
