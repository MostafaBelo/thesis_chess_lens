...
class BoardExtractor:
    ...
    def warp(self, quad, padding=(0, 0)):
        ...
        dst = np.array([
            [padding[0], padding[1]],
            [img_size[0]+padding[0], padding[1]],
            [img_size[0]+padding[0], img_size[1]+padding[1]],
            [padding[0], img_size[1]+padding[1]]
        ], dtype=np.float32)

        M = cv2.getPerspectiveTransform(quad, dst)
        warpped = cv2.warpPerspective(
            self.img.numpy(), M, (img_size[0]+2*padding[0], img_size[1]+2*padding[1]))
